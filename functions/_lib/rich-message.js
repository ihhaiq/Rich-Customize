import { dataRichText, plainRichText } from '../../lib/rich-text.js';

const INPUT_NATIVE_TYPE_ALIASES = Object.freeze({
  text: 'paragraph', paragraph: 'paragraph', caption: 'paragraph',
  section_heading: 'heading', heading: 'heading',
  preformatted: 'pre', pre: 'pre', footer: 'footer', divider: 'divider',
  mathematical_expression: 'mathematical_expression', anchor: 'anchor',
  list: 'list', block_quotation: 'blockquote', blockquote: 'blockquote',
  expandable_block_quotation: 'expandable_blockquote',
  expandable_blockquote: 'expandable_blockquote',
  pull_quotation: 'pullquote', pullquote: 'pullquote',
  collage: 'collage', slideshow: 'slideshow', table: 'table',
  details: 'details', map: 'map', buttons: 'buttons',
  animation: 'animation', audio: 'audio', document: 'document',
  photo: 'photo', video: 'video', voice: 'voice_note',
  voice_note: 'voice_note', thinking: 'thinking',
});

function canonicalInputNativeType(value) {
  const type = String(value || '').trim();
  return type ? (INPUT_NATIVE_TYPE_ALIASES[type] || type) : '';
}
...
function inputMediaPayload(type, source, hasSpoiler = null) {
  ...
  if (type === 'voice_note') defined(media, 'duration', source.duration);
  return media;
}

function normalizeNativeInputBlock(raw, fallbackType = '') {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('invalid native rich block payload');
  const payload = JSON.parse(JSON.stringify(raw));
  const resolvedType = canonicalInputNativeType(payload.type) || canonicalInputNativeType(fallbackType);
  if (!resolvedType) throw new Error('legacy native rich block is missing type');
  payload.type = resolvedType;
  if (Array.isArray(payload.blocks)) payload.blocks = payload.blocks.map((child) => normalizeNativeInputBlock(child));
  if (payload.type === 'list' && Array.isArray(payload.items)) {
    payload.items = payload.items.map((item) => {
      if (!item || typeof item !== 'object' || Array.isArray(item)) return item;
      const normalized = { ...item };
      if (Array.isArray(normalized.blocks)) normalized.blocks = normalized.blocks.map((child) => normalizeNativeInputBlock(child));
      return normalized;
    });
  }
  const mediaField = {
    photo: 'photo', video: 'video', animation: 'animation',
    audio: 'audio', document: 'document', voice_note: 'voice_note',
  }[payload.type];
  if (!mediaField) return payload;
  const source = nativeMediaSource(payload[mediaField]);
  if (!source) throw new Error('native ' + payload.type + ' block has no reusable media');
...
function anchorName(block) {
  const data = block?.data && typeof block.data === 'object' ? block.data : {};
  return String(data.text || data.name || '').trim();
}

function anchorNavigationRichText(blocks) {
  const links = [];
...
function renderGeneratedBlock(block) {
  ...
  if (kind === 'text' || kind === 'paragraph' || kind === 'caption') return { type: 'paragraph', text: text || '' };
  ...
  if (kind === 'mathematical_expression') return {
  ...
  if (kind === 'list') {
    ...
    return {
      ...
      items: items.map((item) => {
        ...
        const source = item && typeof item === 'object' && !Array.isArray(item) ? item : { text: String(item ?? '') };
        return {
          blocks: [{ type: 'paragraph', text: dataRichText(source, 'rich_text', 'html', 'text') || '' }],
  ...
  if (kind === 'table') {
    ...
    if (!rows.length || !rows.every((row) => Array.isArray(row) && row.length)) throw new Error('table block has no cells');
    return {
      type: 'table',
      cells: rows.map((row) => row.map((raw) => {
        const source = raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : { text: String(raw ?? '') };
        const cell = {
  ...
  if (kind === 'details' || kind === 'collage' || kind === 'slideshow') {
    ...
    const payload = { type: kind, blocks: children.flatMap((child) => renderBlockPayloads(child)) };
    if (kind === 'details') {
      payload.summary = dataRichText(data, 'summary_rich_text', 'summary_html', 'summary_text') || '…';
...
function renderBlockPayloads(block) {
  ...
  if (data.native && data.native_data && typeof data.native_data === 'object' && !Array.isArray(data.native_data)) {
    return [normalizeNativeInputBlock(data.native_data, canonicalInputNativeType(data.native_type) || canonicalInputNativeType(kind))];
  }
...
function resolveInlinePageCallbacks(value, sourcePageId = null, navigationToken = null) {
  if (Array.isArray(value)) return value.map((item) => resolveInlinePageCallbacks(item, sourcePageId, navigationToken));
  if (!value || typeof value !== 'object') return value;
  const payload = {};
  for (const [key, item] of Object.entries(value)) payload[key] = resolveInlinePageCallbacks(item, sourcePageId, navigationToken);
...
export function buildInputRichMessage(blocks, { sourcePageId = null } = {}) {
  if (!Array.isArray(blocks) || !blocks.length) throw new Error('The rich message has no blocks');
  ...
  if (anchorNavigation != null) payloads.push({ type: 'paragraph', text: anchorNavigation });
  payloads.push(...ordered.flatMap((block) => renderBlockPayloads(block)));
  return { blocks: resolveInlinePageCallbacks(payloads, sourcePageId, null) };
}
