...
import { detailsChildren, addDetailsChild, replaceDetailsChild } from 'lib/details';
...
async function handleAddingBlock(message,storageKey,session){
  ...
  if(kind==='details'){
    ...
    const child=messageToBlocks(message)[0];if(!child)return send(message,t('details.unsupported_content',{language:lang(message)}));
    const children=[...(data.details_children??[])];children.push(child);
    await patchSession(storageKey,{details_children:children},{state:STATE.ADDING_BLOCK});
...
async function handleEditingBlock(message,storageKey,session){
  const data=session.data,draft=await loadDraft(storageKey),blockId=String(data.edit_block_id??data.current_block_id??'');
...
async function handleEditingButton(message,storageKey,session){
  ...
  await saveDraft(storageKey,draft);await patchSession(storageKey,{pending_button_action:null},{state:STATE.MANAGING});return renderEditor(message,storageKey,'تم تحديث الزر.');
}
  ...
  return withIdempotency(Number(ctx?.update?.update_id),async()=>{
    ...
    try{
      ...
      if(session.state===STATE.WAITING_INPUT){
        const blocks=messageToBlocks(message);if(!blocks.length)return send(message,t('unsupported',{language}));
        const markers=findUserButtonMarkers(message.text);
        if(markers.length){await patchSession(storageKey,{pending_user_blocks:blocks,pending_user_markers:markers,pending_user_marker_index:0},{state:STATE.SELECTING_BUTTON_USER});return send(message,'اختار مستخدم للزر.',{keyboard:[[{text:'اختيار مستخدم',request_users:{request_id:1,user_is_bot:false,max_quantity:1}}]],resize_keyboard:true,one_time_keyboard:true});}
        return openEditorFromBlocks(message,storageKey,blocks);
      }
