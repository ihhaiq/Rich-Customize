...
const LEGACY_NATIVE_TYPES = Object.freeze({
  text: 'paragraph',
  paragraph: 'paragraph',
  caption: 'paragraph',
  voice: 'voice_note',
  blockquote: 'block_quotation',
  pullquote: 'pull_quotation',
});

function legacyNativeType(blockType, nativeType) {
  const explicit = String(nativeType || '').trim();
  if (explicit) return explicit;
  const kind = String(blockType || '').trim();
  return LEGACY_NATIVE_TYPES[kind] || kind;
}
...
function normalizeNativeInputBlock(raw, fallbackType = '') {
  ...
  if (!String(payload.type || '').trim()) {
    const inferred = String(fallbackType || '').trim();
    if (!inferred) throw new Error('legacy native rich block is missing type');
  ...
  if (String(payload.type) === 'list' && Array.isArray(payload.items)) {
