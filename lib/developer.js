...
async function sendPanel(chatId, content = 'لوحة المطور', extraButtons = []) {
  const replyMarkup = panelKeyboard();
  try {
    await api.sendRichMessage({
      chat_id: chatId,
      rich_message: panelRichMessage(content, extraButtons),
    ...
    await api.sendMessage({ chat_id: chatId, text: fallbackText, reply_markup: replyMarkup });
  }
}
...
async function showUsers(query, chatId, requestedPage) {
  ...
  await api.answerCallbackQuery({ callback_query_id: query.id });
  await sendPanel(chatId, blocks, buttons);
}
