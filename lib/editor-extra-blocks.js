...
async function handleSlideshowCallback(query, session, data, languageCode) {
  ...
  const upload = clone(session.addPayload?.slideshow || {});
  const token = parts[3];
  if (
  ...
  if (parts[2] === 'more') {
    ...
    await sendPrompt(
      ...
      slideshowKeyboard(token, upload.children.length, session.pendingAddType === 'details', languageCode),
    );
    await answer(query);
    return true;
  }
...
async function handleDetailsManagerCallback(query, session, data, languageCode) {
  ...
  if (data.startsWith('r:dif:')) {
    ...
    if (!child || !['caption', 'credit', 'add_footer'].includes(action)) return false;
    await setEditorState(userId, 'editing_block', {
      nestedDetailsId: parts[2],
      nestedChildId: parts[3],
      nestedAction: action,
...
async function handleSlideshowMessage(message, session, languageCode, nested = false) {
  ...
  await sendPrompt(
    message.from.id,
    message.chat.id,
    (ar(languageCode) ? 'تم الاستلام: ' : 'Received: ') + upload.children.length + '/' + SLIDESHOW_LIMIT,
    slideshowKeyboard(upload.token, upload.children.length, nested, languageCode),
  );
  return true;
}
