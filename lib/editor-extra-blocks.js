...
function collageConfirmationKeyboard(token, languageCode) {
  return {
    inline_keyboard: [
      [{
      ...
      [{ text: editorCopy(languageCode).back, callback_data: 'r:back' }],
    ],
  };
}
...
async function mergeSlideshowActiveGroup(userId, upload) {
  ...
  const grouped = mediaChildrenFromBlocks(await readAlbumBlocks(userId, groupId));
...
async function handleSlideshowCallback(query, session, data, languageCode) {
  ...
  upload = await mergeSlideshowActiveGroup(query.from.id, upload);
...
async function handleCollageCallback(query, session, data, languageCode) {
  ...
  const children = mediaChildrenFromBlocks(
    await readAlbumBlocks(query.from.id, upload.group_id),
  );
  if (!children.length) {
