...
async function handleAlbumFlowCallback(query, session, data, languageCode) {
  ...
  if (
    ...
    await answer(query, ar(languageCode) ? 'انتهت صلاحية الألبوم.' : 'This album flow has expired.', true);
    return true;
  }
  ...
  if (!batch.blocks.length) {
    await answer(query, ar(languageCode) ? 'ماكو عناصر محفوظة بالألبوم.' : 'No album items were saved.', true);
    return true;
  }
  ...
  if (flow.mode === 'add_quote') {
    ...
    if (
      ...
      await answer(query, ar(languageCode) ? 'انتهت عملية الاقتباس.' : 'Quote flow expired.', true);
      return true;
    }
    const replacement = quoteReplacementFromBlocks(batch.blocks);
    if (!replacement) {
      await answer(query, ar(languageCode) ? 'الألبوم لا يحتوي وسائط مدعومة.' : 'The album has no supported media.', true);
      return true;
    }
  ...
  if (flow.mode === 'add_details_quote') {
    ...
    if (
      ...
      await answer(query, ar(languageCode) ? 'انتهت عملية الاقتباس الداخلي.' : 'Inner quote flow expired.', true);
      return true;
    }
    const replacement = quoteReplacementFromBlocks(batch.blocks);
    if (!replacement) {
  ...
  if (flow.mode === 'add_details_content') {
    if (
      ...
      await answer(query, ar(languageCode) ? 'انتهت عملية Details.' : 'Details flow expired.', true);
      return true;
    }
    const children = Array.isArray(payload.children) ? payload.children : [];
  ...
  if (flow.mode === 'edit_container') {
    ...
    if (
      ...
      await answer(query, ar(languageCode) ? 'تعذر اعتماد الألبوم لهذا البلوك.' : 'Could not apply this album to the block.', true);
      return true;
    }
  ...
  if (flow.mode === 'edit_quote') {
    ...
    if (
      ...
      await answer(query, ar(languageCode) ? 'تعذر اعتماد الألبوم لهذا الاقتباس.' : 'Could not apply this album to the quote.', true);
      return true;
    }
  ...
  if (flow.mode === 'edit_details_content') {
    ...
    if (
      ...
      await answer(query, ar(languageCode) ? 'تعذر اعتماد الألبوم داخل Details.' : 'Could not apply this album to Details.', true);
      return true;
    }
    ...
    if (!incoming.length) {
      await answer(query, ar(languageCode) ? 'الألبوم لا يحتوي محتوى مدعوم.' : 'The album has no supported content.', true);
      return true;
    }
    detachDetails(details);
  ...
  if (flow.mode === 'edit_nested_container' || flow.mode === 'edit_nested_quote') {
    ...
    if (
      ...
      await answer(query, ar(languageCode) ? 'تعذر اعتماد الألبوم للبلوك الداخلي.' : 'Could not apply this album to the inner block.', true);
      return true;
    }
    ...
    if (!candidate) {
      await answer(query, ar(languageCode) ? 'الألبوم لا يطابق نوع البلوك.' : 'The album does not match this block type.', true);
      return true;
    }

    const selected = replaceDetailsChild(details, flow.child_id, candidate);
    if (!selected) {
      await answer(query, ar(languageCode) ? 'البلوك الداخلي لم يعد موجودًا.' : 'The inner block no longer exists.', true);
      return true;
    }
    const saved = await saveMutatedBlocks(query.from.id, session, blocks, {
      state: 'managing',
  ...
  await answer(query, ar(languageCode) ? 'نوع عملية الألبوم غير معروف.' : 'Unknown album operation.', true);
  return true;
}
  ...
  if (type === 'mathematical_expression' && message?.rich_message) {
    ...
    if (!ready) {
      await api.sendMessage({
        chat_id: message.chat.id,
        text: ar(languageCode)
          ? 'الرسالة الغنية لا تحتوي بلوك معادلة جاهز.'
          : 'The rich message does not contain a ready mathematical-expression block.',
      });
      return true;
    }
