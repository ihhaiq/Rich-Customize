...
function localeOf(source) {
  return resolveLanguage(typeof source === 'string' ? source : code(source));
}
...
function limitText(result, languageCode) {
  const locale = localeOf(languageCode);
...
async function replaceTopBlock(userId, session, message, block, replacement, languageCode) {
  ...
  if (!updated) {
    await api.sendMessage({
      ...
      text: tr(localeOf(languageCode), 'This block no longer exists.'),
    });
    return false;
  }
  alignLinkedAnchors(blocks);
...
function promptForAdd(type, languageCode) {
  const locale = localeOf(languageCode);
  const prompts = {
    mathematical_expression: t(locale, 'math.add_prompt'),
    anchor: t(locale, 'anchor.name_required'),
    list: t(locale, 'list.bullet_prompt'),
    table: tr(locale, 'Send table rows, one per line, separating columns with |.'),
    blockquote: t(locale, 'details.send_quote'),
    pullquote: t(locale, 'details.send_pullquote'),
    collage: t(locale, 'details.send_collage'),
    slideshow: t(locale, 'details.send_slideshow'),
    map: t(locale, 'details.send_map'),
    animation: t(locale, 'details.send_animation'),
    audio: t(locale, 'details.send_audio'),
    document: t(locale, 'details.send_document'),
    photo: t(locale, 'details.send_photo'),
    video: t(locale, 'details.send_video'),
    voice: t(locale, 'details.send_voice'),
  };
  return prompts[type] || tr(locale, 'Send the content.');
}

function promptForEdit(type, languageCode) {
  const locale = localeOf(languageCode);
  const prompts = {
    mathematical_expression: t(locale, 'math.edit_prompt'),
    anchor: tr(locale, 'Send the new anchor display name.'),
    list: tr(locale, 'Send the new list items, one per line.'),
    table: tr(locale, 'Send the new table rows, separating columns with |.'),
    blockquote: tr(locale, 'Send new quote text or new media/file.'),
    pullquote: tr(locale, 'Send new pull quote text or new media/file.'),
    collage: tr(locale, 'Send new photos/videos for the collage.'),
    slideshow: tr(locale, 'Send new photos/videos for the slideshow.'),
    map: tr(locale, 'Send the new location.'),
    animation: tr(locale, 'Send a new GIF.'),
    audio: tr(locale, 'Send new audio.'),
    document: tr(locale, 'Send the new document.'),
    photo: tr(locale, 'Send the new photo.'),
    video: tr(locale, 'Send the new video.'),
    voice: tr(locale, 'Send the new voice note.'),
    details: t(locale, 'details.replace_content_prompt'),
  };
  return prompts[type] || tr(locale, 'Send replacement content of the same type.');
}

function readyRichBlock(message, type) {
  if (!message?.rich_message) return null;
  const wanted = String(type || '');
  return messageToBlocks(message)
    .find((item) => String(item?.type || '') === wanted) || null;
}
...
function collageConfirmationKeyboard(token, languageCode, nested = false) {
  return {
    inline_keyboard: [
      [{
        text: tr(localeOf(languageCode), 'Continue'),
...
async function queueAlbumFlow(
  ...
  if (
    ...
    await api.sendMessage({
      chat_id: message.chat.id,
      text: tr(localeOf(languageCode), 'Finish the current album first.'),
    });
    return true;
  }
  ...
  await sendPrompt(
    message.from.id,
    message.chat.id,
    (tr(localeOf(languageCode), 'Album items received: '))
...
function detailsBuilderText(payload, languageCode) {
  ...
  return t(localeOf(languageCode), 'details.builder_text', { count });
}

function detailsInnerListText(details, languageCode) {
  const children = detailsChildren(details);
  const lines = [
    tr(localeOf(languageCode), '📂 Inner blocks'),
    (tr(localeOf(languageCode), 'Count: ')) + children.length,
    '',
  ];
  children.forEach((child, index) => {
    lines.push((index + 1) + '. ' + String(child?.type || 'content'));
  });
  lines.push('', tr(localeOf(languageCode), 'Choose an action:'));
  return lines.join('\n');
}
...
function detailsInnerPage(details, child, languageCode) {
  ...
  return [
    tr(localeOf(languageCode), 'Inner block settings'),
    (tr(localeOf(languageCode), 'Type: ')) + String(child?.type || 'content'),
    (tr(localeOf(languageCode), 'Position: ')) + (index + 1) + '/' + children.length,
    '',
    tr(localeOf(languageCode), 'Choose an action:'),
  ].join('\n');
}
...
async function handleAddCallback(query, session, data, languageCode) {
  ...
  if (data === 'r:add:listmenu') {
    await editMessage(
      chatId,
      messageId,
      tr(localeOf(languageCode), 'Choose list type:'),
  ...
  if (data === 'r:add:details') {
    ...
    await sendPrompt(
      userId,
      chatId,
      tr(localeOf(languageCode), 'Send the Details summary.'),
    );
    await answer(query);
    return true;
  }
  ...
  if (type === 'anchor') {
    ...
    if (!targets.length) {
      await answer(
        query,
        tr(localeOf(languageCode), 'Add at least one content block before creating an anchor.'),
        true,
      );
      return true;
    }
...
async function handleAnchorTargetCallback(query, session, data, languageCode) {
  ...
  if (session.state !== 'adding_block' || session.pendingAddType !== 'anchor' || session.addStep !== 'anchor_target') {
    await answer(query, tr(localeOf(languageCode), 'Anchor flow expired.'), true);
    return true;
  }
  ...
  if (!target || target.type === 'anchor' || !displayName) {
    await answer(query, tr(localeOf(languageCode), 'Invalid selection.'), true);
    return true;
  }
  ...
  await answer(query, tr(localeOf(languageCode), 'Anchor added.'));
  return true;
}
...
async function handleSlideshowCallback(query, session, data, languageCode) {
  ...
  if (
    ...
    await answer(query, tr(localeOf(languageCode), 'This flow has expired.'), true);
    return true;
  }
  ...
  if (!merged.ready) {
    await answer(
      query,
      tr(localeOf(languageCode), 'The album is still receiving items. Try again in a moment.'),
      true,
    );
    return true;
  }
  ...
  if (!upload.children.length) {
    await answer(query, tr(localeOf(languageCode), 'No media has been saved yet.'), true);
    return true;
  }

  if (parts[2] === 'more') {
    if (upload.children.length >= SLIDESHOW_LIMIT) {
      await answer(query, tr(localeOf(languageCode), 'The slideshow reached 50 items.'), true);
      return true;
    }
    const payload = clone(session.addPayload || {});
...
async function handleCollageCallback(query, session, data, languageCode) {
  ...
  if (!children.length) {
    await answer(query, tr(localeOf(languageCode), 'No media was saved.'), true);
    return true;
  }
...
async function handleAlbumFlowCallback(query, session, data, languageCode) {
  if (!data.startsWith('r:album:done:')) return false;
  const token = data.slice('r:album:done:'.length);
  const flow = clone(session.addPayload?.album_flow || {});
  if (
    !flow.token
    || !flow.group_id
    || !flow.mode
    || String(flow.token) !== String(token)
  ) {
    await answer(query, tr(localeOf(languageCode), 'This album flow has expired.'), true);
    return true;
  }

  const batch = await readAlbumBatch(query.from.id, flow.group_id);
  if (!batch.quiet) {
    await answer(
      query,
      tr(localeOf(languageCode), 'The album is still receiving items. Try again in a moment.'),
  ...
  if (!batch.blocks.length) {
    await answer(query, tr(localeOf(languageCode), 'No album items were saved.'), true);
    return true;
  }

  const payload = clone(session.addPayload || {});
  delete payload.album_flow;

  if (flow.mode === 'add_quote') {
    const quoteType = String(flow.quote_type || session.pendingAddType || '');
    if (
      session.state !== 'adding_block'
      || session.pendingAddType !== quoteType
      || !QUOTE_TYPES.includes(quoteType)
    ) {
      await answer(query, tr(localeOf(languageCode), 'Quote flow expired.'), true);
      return true;
    }
    const replacement = quoteReplacementFromBlocks(batch.blocks);
    if (!replacement) {
      await answer(query, tr(localeOf(languageCode), 'The album has no supported media.'), true);
      return true;
    }
    Object.assign(payload, replacement);
    const hasCaption = Boolean(
      replacement.quote_text
      || replacement.quote_html
      || replacement.quote_rich_text,
    );
    await clearAlbum(query.from.id, flow.group_id);
    await clearInputMessages(session, query.message);
    await updateEditorSession(query.from.id, {
      addStep: hasCaption ? 'quote_credit' : 'quote_media_text',
      addPayload: payload,
    });
    await sendPrompt(
      query.from.id,
      query.message.chat.id,
      hasCaption
        ? (tr(localeOf(languageCode), 'Media caption used as quote text. Send author or /skip.'))
        : (tr(localeOf(languageCode), 'Now send the quote text.')),
    );
    await answer(query);
    return true;
  }

  if (flow.mode === 'add_details_quote') {
    const quoteType = String(flow.quote_type || session.pendingChildType || '');
    if (
      session.state !== 'adding_block'
      || session.pendingAddType !== 'details'
      || session.pendingChildType !== quoteType
      || !QUOTE_TYPES.includes(quoteType)
    ) {
      await answer(query, tr(localeOf(languageCode), 'Inner quote flow expired.'), true);
      return true;
    }
    const replacement = quoteReplacementFromBlocks(batch.blocks);
    if (!replacement) {
    ...
    payload.child_media_children = replacement.media_children || [];
    if (replacement.quote_text != null) payload.child_quote_text = replacement.quote_text;
    if (replacement.quote_html != null) payload.child_quote_html = replacement.quote_html;
    if (replacement.quote_rich_text != null) payload.child_quote_rich_text = replacement.quote_rich_text;
    const hasCaption = Boolean(
      payload.child_quote_text
      || payload.child_quote_html
      || payload.child_quote_rich_text,
    );
    await clearAlbum(query.from.id, flow.group_id);
    await clearInputMessages(session, query.message);
    await updateEditorSession(query.from.id, {
      addStep: hasCaption ? 'details_child_quote_credit' : 'details_child_pullquote_text',
      addPayload: payload,
    });
    await sendPrompt(
      query.from.id,
      query.message.chat.id,
      hasCaption
        ? (tr(localeOf(languageCode), 'Send the author or /skip.'))
        : (tr(localeOf(languageCode), 'Send quote text.')),
    );
    await answer(query);
    return true;
  }

  if (flow.mode === 'add_details_content') {
    if (
      session.state !== 'adding_block'
      || session.pendingAddType !== 'details'
      || session.addStep !== 'details_content'
    ) {
      await answer(query, tr(localeOf(languageCode), 'Details flow expired.'), true);
      return true;
    }
    const children = Array.isArray(payload.children) ? payload.children : [];
    for (const child of batch.blocks) addBlock(children, child);
    payload.children = children;
    await clearAlbum(query.from.id, flow.group_id);
    const liveSession = { ...session, addPayload: payload };
    await finishAdd(
      query.from.id,
      liveSession,
      query.message,
  ...
  if (flow.mode === 'edit_container') {
    const block = getBlockById(session.blocks, flow.block_id);
    const type = String(flow.expected_type || block?.type || '');
    const children = mediaChildrenFromBlocks(batch.blocks);
    if (
      session.state !== 'editing_block'
      || !block
      || block.type !== type
      || !CONTAINER_TYPES.has(type)
      || !children.length
    ) {
      await answer(query, tr(localeOf(languageCode), 'Could not apply this album to the block.'), true);
      return true;
    }
    const replacement = { ...stripNative(block.data || {}), children };
    const liveSession = { ...session, addPayload: payload };
    const ok = await replaceTopBlock(
      query.from.id,
      liveSession,
      query.message,
      block,
    ...
    if (ok) await clearAlbum(query.from.id, flow.group_id);
    await answer(query);
    return true;
  }

  if (flow.mode === 'edit_quote') {
    const block = getBlockById(session.blocks, flow.block_id);
    const type = String(flow.expected_type || block?.type || '');
    if (
      session.state !== 'editing_block'
      || !block
      || block.type !== type
      || !QUOTE_TYPES.includes(type)
    ) {
      await answer(query, tr(localeOf(languageCode), 'Could not apply this album to the quote.'), true);
      return true;
    }
    const replacement = quoteReplacementFromBlocks(batch.blocks, block.data || {});
    if (!replacement) {
      await answer(query, tr(localeOf(languageCode), 'The album has no supported media.'), true);
      return true;
    }
  ...
  if (flow.mode === 'edit_details_content') {
    const blocks = clone(session.blocks || []);
    const details = getBlockById(blocks, flow.block_id);
    if (
      session.state !== 'editing_block'
      || !details
      || details.type !== 'details'
    ) {
      await answer(query, tr(localeOf(languageCode), 'Could not apply this album to Details.'), true);
      return true;
    }
    let incoming = clone(batch.blocks);
    if (incoming.length === 1 && incoming[0]?.type === 'details') {
      incoming = clone(incoming[0]?.data?.children || []);
    }
    if (!incoming.length) {
      await answer(query, tr(localeOf(languageCode), 'The album has no supported content.'), true);
      return true;
    }
    detachDetails(details);
    details.data.children = incoming;
    normalizeBlocks(details.data.children);
    ...
    await clearAlbum(query.from.id, flow.group_id);
    await clearInputMessages(session, query.message);
    const current = getBlockById(saved.updated.blocks, flow.block_id);
    await editManagement(
      saved.updated,
  ...
  if (flow.mode === 'edit_nested_container' || flow.mode === 'edit_nested_quote') {
    const blocks = clone(session.blocks || []);
    const details = getBlockById(blocks, flow.details_id);
    const child = details ? findDetailsChild(details, flow.child_id) : null;
    const type = String(flow.expected_type || child?.type || '');
    if (
      session.state !== 'editing_block'
      || !details
      || details.type !== 'details'
      ...
      await answer(query, tr(localeOf(languageCode), 'Could not apply this album to the inner block.'), true);
      return true;
    }

    let candidate = null;
    if (flow.mode === 'edit_nested_container' && CONTAINER_TYPES.has(type)) {
      const children = mediaChildrenFromBlocks(batch.blocks);
      if (children.length) {
        candidate = makeBlock(type, {
          ...stripNative(child.data || {}),
          children,
        });
      }
    } else if (flow.mode === 'edit_nested_quote' && QUOTE_TYPES.includes(type)) {
      const replacement = quoteReplacementFromBlocks(batch.blocks, child.data || {});
      if (replacement) candidate = makeBlock(type, replacement);
    }
    if (!candidate) {
      await answer(query, tr(localeOf(languageCode), 'The album does not match this block type.'), true);
      return true;
    }

    const selected = replaceDetailsChild(details, flow.child_id, candidate);
    if (!selected) {
      await answer(query, tr(localeOf(languageCode), 'The inner block no longer exists.'), true);
      return true;
    }
    const saved = await saveMutatedBlocks(query.from.id, session, blocks, {
      state: 'managing',
    ...
    await clearAlbum(query.from.id, flow.group_id);
    await clearInputMessages(session, query.message);
    const currentDetails = getBlockById(saved.updated.blocks, flow.details_id);
    const currentChild = findDetailsChild(currentDetails, selected.id);
    await editManagement(
      saved.updated,
      detailsInnerPage(currentDetails, currentChild, languageCode),
      buildDetailsInnerBlockKeyboard(currentDetails, currentChild, languageCode),
    );
    await answer(query);
    return true;
  }

  await answer(query, tr(localeOf(languageCode), 'Unknown album operation.'), true);
  return true;
}
...
async function handleFieldCallback(query, session, data, languageCode) {
  ...
  const block = getBlockById(session.blocks, id);
  if (!block) {
    await answer(query, tr(localeOf(languageCode), 'This block no longer exists.'), true);
    return true;
  }
  if (block.type === 'details' && field === 'summary') {
    ...
    await sendPrompt(query.from.id, query.message.chat.id, tr(localeOf(languageCode), 'Send the new Details summary.'));
    await answer(query);
    return true;
  }
  const allowed = (
  ...
  await sendPrompt(
    ...
    query.message.chat.id,
    field === 'caption'
      ? (tr(localeOf(languageCode), 'Send the new caption, or /remove.'))
      : (tr(localeOf(languageCode), 'Send the new author/source, or /remove.')),
  );
  await answer(query);
  return true;
}
...
async function handleChecklistCallback(query, session, data, languageCode) {
  ...
  if (
    ...
    await answer(query, tr(localeOf(languageCode), 'This task no longer exists.'), true);
    return true;
  }
  const blocks = clone(session.blocks);
...
async function handleTableCallback(query, session, data, languageCode) {
  ...
  if (data.startsWith('r:tm:')) {
    ...
    if (!block || block.type !== 'table' || !tableRows(block).length) {
      await answer(query, tr(localeOf(languageCode), 'This table is missing or has no cells.'), true);
      return true;
    }
    await editMessage(chatId, messageId, tr(localeOf(languageCode), 'Table cell settings\n\nChoose an action:'), buildTableOptionsKeyboard(id, languageCode));
    await answer(query);
    return true;
  }
  ...
  if (data.startsWith('r:ta:')) {
    ...
    if (all[action]) {
      ...
      if (!table || !setAllTableCellsStyle(table, all[action])) {
        await answer(query, tr(localeOf(languageCode), 'Could not edit the table.'), true);
        return true;
      }
      ...
      await editMessage(chatId, messageId, tr(localeOf(languageCode), 'Table cell settings\n\nChoose an action:'), buildTableOptionsKeyboard(id, languageCode));
      await answer(query, tr(localeOf(languageCode), 'All cells updated.'));
      return true;
    }
    if (single[action]) {
      await editMessage(chatId, messageId, tr(localeOf(languageCode), 'Choose the cell:'), buildTableCellKeyboard(block, action, languageCode));
      await answer(query);
      return true;
    }
    return false;
  ...
  if (data.startsWith('r:tc:')) {
    ...
    if (!table || !setTableCellStyle(table, row, column, settings)) {
      await answer(query, tr(localeOf(languageCode), 'This cell no longer exists.'), true);
      return true;
    }
    replaceBlockData(blocks, id, table, 'generated');
    const saved = await saveMutatedBlocks(userId, session, blocks);
    ...
    await editMessage(chatId, messageId, tr(localeOf(languageCode), 'Table cell settings\n\nChoose an action:'), buildTableOptionsKeyboard(id, languageCode));
    await answer(query, tr(localeOf(languageCode), 'Cell updated.'));
    return true;
  }
  ...
  if (data.startsWith('r:tdisplay:')) {
    ...
    await editMessage(
      chatId,
      messageId,
      tr(localeOf(languageCode), '🧱 Table appearance\n\nChoose a property to change.'),
  ...
  if (data.startsWith('r:tcaption:')) {
    ...
    await sendPrompt(
      userId,
      chatId,
      tr(localeOf(languageCode), 'Send the table caption. Send /empty to remove it.'),
    );
    await answer(query);
    return true;
  }
...
async function handleDetailsBuilderCallback(query, session, data, languageCode) {
  ...
  if (data === 'r:details:add') {
    ...
    await editMessage(
      chatId,
      messageId,
      tr(localeOf(languageCode), 'Choose the inner block type:'),
  ...
  if (data === 'r:details:cancel') {
    ...
    await answer(query, tr(localeOf(languageCode), 'Cancelled.'));
    return true;
  }
  ...
  if (data === 'r:details:finish') {
    ...
    if (!summaryHtml || !children.length) {
      await answer(query, tr(localeOf(languageCode), 'Add at least one inner block.'), true);
      return true;
    }
  ...
  if (data.startsWith('r:hs:details:')) {
    ...
    await editMessage(
      chatId,
      messageId,
      (tr(localeOf(languageCode), 'Selected H')) + level + '. ' + (tr(localeOf(languageCode), 'Send the text now.')),
      { inline_keyboard: [[{ text: editorCopy(languageCode).back, callback_data: 'r:details:add' }]] },
    );
    await answer(query);
    return true;
  ...
  if (data.startsWith('r:details:type:')) {
    ...
    if (type === 'heading') {
      ...
      await editMessage(
        chatId,
        messageId,
        tr(localeOf(languageCode), 'Choose heading level:'),
...
async function handleDetailsManagerCallback(query, session, data, languageCode) {
  ...
  if (data.startsWith('r:dip:')) {
    ...
    try {
      ...
      await answer(query, tr(localeOf(languageCode), 'Preview failed.'), true);
    }
    return true;
  }
  ...
  if (data.startsWith('r:dif:')) {
    ...
    if (!allowed) {
      await answer(query, tr(localeOf(languageCode), 'This field is no longer available.'), true);
      return true;
    }
    await setEditorState(userId, 'editing_block', {
      nestedDetailsId: parts[2],
    ...
    const prompt = action === 'caption'
      ? (tr(localeOf(languageCode), 'Send the caption or /remove.'))
      : action === 'credit'
        ? (tr(localeOf(languageCode), 'Send the source/author or /remove.'))
        : (tr(localeOf(languageCode), 'Send footer text.'));
  ...
  if (data.startsWith('r:did:')) {
    ...
    await editMessage(
      chatId,
      messageId,
      tr(localeOf(languageCode), 'Delete this inner block?'),
  ...
  if (data.startsWith('r:didok:')) {
    ...
    await answer(query, tr(localeOf(languageCode), 'Deleted.'));
    return true;
  }
  ...
  if (data.startsWith('r:dimu:') || data.startsWith('r:dimd:')) {
    ...
    if (!details || !moveDetailsChild(details, parts[3], up ? -1 : 1)) {
      await answer(query, tr(localeOf(languageCode), 'Already at this edge.'));
      return true;
    }
    const saved = await saveMutatedBlocks(userId, session, blocks);
    if (!saved.ok) {
...
export async function handleExtraBlockCallback(query, session) {
  ...
  if (await handleAlbumFlowCallback(query, session, data, languageCode)) return true;
...
async function handleSlideshowMessage(message, session, languageCode, nested = false) {
  ...
  if (!validateSlideshowMessage(message)) {
    await api.sendMessage({
      chat_id: message.chat.id,
      text: tr(localeOf(languageCode), 'This media exceeds the slideshow safety limits.'),
    });
    return true;
  }
  ...
  if (message?.media_group_id) {
    ...
    if (upload.active_group && String(upload.active_group) !== collected.groupId) {
      await api.sendMessage({
        chat_id: message.chat.id,
        text: tr(localeOf(languageCode), 'Finish the current batch first with Continue or Add more.'),
      });
      return true;
    }
    ...
    if (!children.length) {
      await api.sendMessage({
        chat_id: message.chat.id,
        text: tr(localeOf(languageCode), 'Send photos or videos.'),
      });
      return true;
    }
  ...
  await sendPrompt(
    message.from.id,
    message.chat.id,
    (tr(localeOf(languageCode), 'Received: ')) + visibleCount + '/' + SLIDESHOW_LIMIT,
...
async function handleAddMessage(message, session, languageCode) {
  ...
  if (type === 'mathematical_expression' && message?.rich_message) {
    const ready = readyRichBlock(message, type);
    if (!ready) {
      await api.sendMessage({
        chat_id: message.chat.id,
        text: tr(localeOf(languageCode), 'The rich message does not contain a ready mathematical-expression block.'),
      });
      return true;
    }
    await finishAdd(userId, session, message, ready, languageCode);
    return true;
  }
  ...
  if (type === 'anchor') {
    if (step === 'content') {
      ...
      if (!display) {
        await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send an anchor name.') });
        return true;
      }
      const targets = (session.blocks || []).filter((block) => block?.type !== 'anchor');
      if (!targets.length) {
        await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'There is no target block.') });
        return true;
      }
      await clearInputMessages(session, message);
      await updateEditorSession(userId, {
      ...
      await sendPrompt(
        userId,
        message.chat.id,
        t(localeOf(languageCode), 'anchor.choose_target', { name: display }),
        buildAnchorTargetKeyboard(latest.blocks, languageCode, 'r:at', 'r:addmenu'),
      );
      return true;
    }
    await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Choose the target using the buttons.') });
    return true;
  }
  ...
  if (QUOTE_TYPES.includes(type)) {
    if (step === 'quote_text') {
      if (typeof message?.text === 'string' && message.text) {
        ...
        await sendPrompt(userId, message.chat.id, tr(localeOf(languageCode), 'Send the author, or /skip.'));
        return true;
      }
      if (message?.media_group_id) {
        return queueAlbumFlow(
          message,
          session,
          'add_quote',
          languageCode,
      ...
      const [media, caption] = quoteMediaPayload(parsed);
      if (!media.length) {
        await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send text or supported media.') });
        return true;
      }
    ...
    if (step === 'quote_media_text') {
      if (!message?.text) {
        await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send the quote text.') });
        return true;
      }
      const next = {
        ...payload,
    ...
    if (step === 'quote_credit') {
      if (!message?.text) {
        await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send the author or /skip.') });
        return true;
      }
  ...
  if (type === 'collage') {
    if (message?.media_group_id) {
      ...
      await sendPrompt(
        userId,
        message.chat.id,
        (tr(localeOf(languageCode), 'Collage items received: ')) + children.length,
    ...
    const children = mediaChildren(message);
    if (!children.length) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send photos or videos.') });
      return true;
    }
  ...
  if (type === 'map') {
    if (!message?.location) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send a Telegram location.') });
      return true;
    }
  ...
  if (MEDIA_TYPES.has(type)) {
    ...
    if (!block) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Wrong media type.') });
      return true;
    }
  ...
  if (SIMPLE_TEXT_TYPES.has(type)) {
    if (!message?.text) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'This block type requires text.') });
      return true;
    }
    ...
    if (type === 'list' && !block.data.items.length) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'The list is empty.') });
      return true;
    }
    await finishAdd(userId, session, message, block, languageCode);
    return true;
...
async function handleDetailsAddMessage(message, session, languageCode) {
  ...
  if (step === 'details_summary') {
    if (!message?.text) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'The summary requires text.') });
      return true;
    }
    await clearInputMessages(session, message);
  ...
  if (step === 'details_child_pullquote_text') {
    if (!message?.text) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send quote text.') });
      return true;
    }
    const next = {
      ...payload,
    ...
    await updateEditorSession(userId, { addStep: 'details_child_quote_credit', addPayload: next });
    await sendPrompt(userId, message.chat.id, tr(localeOf(languageCode), 'Send author or /skip.'));
    return true;
  }

  if (step === 'details_child_quote_credit') {
    if (!message?.text) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send author or /skip.') });
      return true;
    }
    const childType = String(session.pendingChildType || '');
    const credit = message.text.trim().toLowerCase() === '/skip' ? null : messageHtmlText(message);
  ...
  if (step === 'details_child_content') {
    ...
    if (type === 'mathematical_expression' && message?.rich_message) {
      ...
      await storeDetailsChild(userId, session, message, ready, languageCode);
      return true;
    }
    ...
    if (type === 'collage') {
      if (message?.media_group_id) {
        ...
        await sendPrompt(
          ...
          message.chat.id,
          (tr(localeOf(languageCode), 'Collage items received: ')) + children.length,
          collageConfirmationKeyboard(collected.token, languageCode, true),
        );
        return true;
      }
      const children = mediaChildren(message);
    ...
    if (!child) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Content does not match the selected type.') });
      return true;
    }
    await storeDetailsChild(userId, session, message, child, languageCode);
    return true;
  ...
  if (step === 'details_content') {
    ...
    if (!incoming.length) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'This content is not supported inside Details.') });
      return true;
    }
    const children = Array.isArray(payload.children) ? payload.children : [];
...
async function handleNestedEditMessage(message, session, languageCode) {
  ...
  if (!details || !child) {
    ...
    await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'This block no longer exists.') });
    return true;
  }
  ...
  if (
    ...
    && CONTAINER_TYPES.has(String(session.expectedType || child.type || ''))
  ) {
    return queueAlbumFlow(
      message,
      session,
      'edit_nested_container',
      languageCode,
      {
        details_id: detailsId,
        child_id: childId,
        expected_type: String(session.expectedType || child.type || ''),
      },
      'r:di:' + detailsId + ':' + childId,
    );
  }
  if (
    action === 'content'
    && message?.media_group_id
    && QUOTE_TYPES.includes(String(session.expectedType || child.type || ''))
  ) {
    return queueAlbumFlow(
      message,
      session,
  ...
  if (action === 'add_footer') {
    if (!message?.text) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send footer text.') });
      return true;
    }
    const children = detailsChildren(details);
    ...
    if (!message?.text) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send text.') });
      return true;
    }
    ...
    let candidate = null;
    if (type === 'mathematical_expression' && message?.rich_message) {
      candidate = readyRichBlock(message, type);
    } else if (QUOTE_TYPES.includes(type)) {
    ...
    if (!candidate) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Content does not match the block type.') });
      return true;
    }
...
async function handleEditMessage(message, session, languageCode) {
  ...
  if (session.editField === 'table_caption' && type === 'table') {
    if (!message?.text) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Send a caption or /empty.') });
      return true;
    }
    const blocks = clone(session.blocks);
  ...
  if (message?.media_group_id && type === 'details') {
    return queueAlbumFlow(
      message,
      session,
  ...
  if (message?.media_group_id && CONTAINER_TYPES.has(type)) {
    return queueAlbumFlow(
      message,
      session,
      'edit_container',
      languageCode,
      { block_id: String(block.id), expected_type: type },
      'r:b:' + block.id,
    );
  }
  if (message?.media_group_id && QUOTE_TYPES.includes(type)) {
    return queueAlbumFlow(
      message,
      session,
      'edit_quote',
  ...
  if (type === 'details') {
    ...
    if (!incoming.length) {
      await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Unsupported Details content.') });
      return true;
    }
    const replacement = stripNative(block.data || {});
  ...
  if (type === 'mathematical_expression' && message?.rich_message) {
    const ready = readyRichBlock(message, type);
    if (ready) replacement = ready.data;
  } else if (QUOTE_TYPES.includes(type)) {
  ...
  if (!replacement) {
    await api.sendMessage({ chat_id: message.chat.id, text: tr(localeOf(languageCode), 'Wrong content type. Send the same block type.') });
    return true;
  }
