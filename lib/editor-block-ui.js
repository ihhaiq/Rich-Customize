...
const COPY = {
  en: {
    ...
    chooseBlock: 'Choose the block you want to edit:',
    choosePosition: 'Choose the new position:',
    addBlock: '➕ Add block',
    pages: '📚 My Pages',
    preview: '👁 Preview',
    undo: '↩️ Undo',
    redo: '↪️ Redo',
    tools: '🛠 More tools',
    toolsText: '🛠 More tools\n\nChoose the tool you need:',
    manageButtons: '🔘 Manage buttons',
    publish: '📤 Publish',
    currentPositionNotice: 'This is the current position.',
    retry: '🔄 Try again',
    back: '🔙 Back',
    scrollUp: '⬆️ Up',
    scrollDown: '⬇️ Scroll',
    ...
    delete: '🗑 Delete',
    changeTarget: '🎯 Change target block',
    moveAnchorTo: (target) => 'Move the anchor to ' + target,
    deleteWithAnchor: '🗑 Delete the block and linked anchor',
    linkedDeleteWarning: (count) => 'This block has ' + count + ' linked anchor(s). Move them to another block or delete them together.',
    anchorDeleted: 'Block and linked anchor deleted.',
    moveUp: '⬆️ Move up',
  ...
  ar: {
    ...
    chooseBlock: 'اختر البلوك الذي تريد تعديله:',
    choosePosition: 'اختر الموقع الجديد:',
    addBlock: '➕ إضافة بلوك',
    pages: '📚 صفحاتي',
    preview: '👁 معاينة',
    undo: '↩️ تراجع',
    redo: '↪️ إعادة',
    tools: '🛠 أدوات إضافية',
    toolsText: '🛠 أدوات إضافية\n\nاختر الأداة التي تحتاجها:',
    manageButtons: '🔘 إدارة الأزرار',
    publish: '📤 نشر',
    currentPositionNotice: 'هذا هو الموقع الحالي',
    retry: '🔄 حاول مجددًا',
    back: '🔙 رجوع',
    scrollUp: '⬆️ صعود',
    scrollDown: '⬇️ تمرير',
    ...
    delete: '🗑 حذف',
    changeTarget: '🎯 تغيير البلوك الهدف',
    moveAnchorTo: (target) => 'نقل المرساة إلى ' + target,
    deleteWithAnchor: '🗑 حذف البلوك والمرساة المرتبطة',
    linkedDeleteWarning: (count) => 'هذا البلوك مرتبط بعدد ' + count + ' من المراسي. انقلها إلى بلوك آخر أو احذفها معه.',
    anchorDeleted: 'تم حذف البلوك والمرساة المرتبطة.',
    moveUp: '⬆️ للأعلى',
    moveDown: '⬇️ للأسفل',
    yesDelete: '🗑 نعم، حذف',
    cancel: 'إلغاء',
...
export function buildListTypeKeyboard(languageCode, callbackPrefix = 'r:addlist', backData = 'r:addmenu') {
  const ar = editorLanguage(languageCode) === 'ar';
  return {
    inline_keyboard: [
      [{ text: ar ? '• قائمة نقطية' : '• Bulleted list', callback_data: callbackPrefix + ':bullet' }],
      [{ text: ar ? '1. قائمة مرقمة' : '1. Numbered list', callback_data: callbackPrefix + ':numbered' }],
      [{ text: ar ? '☑️ قائمة مهام' : '☑️ Checklist', callback_data: callbackPrefix + ':checklist' }],
      [{ text: editorCopy(languageCode).cancel, callback_data: backData }],
    ],
  };
}
...
export function buildBlockEditorKeyboard(block, blocks, languageCode) {
  ...
  if (block.type === 'details') {
    rows.push([{
      text: editorLanguage(languageCode) === 'ar' ? '✏️ تعديل العنوان' : '✏️ Edit summary',
      callback_data: 'r:f:' + id + ':summary',
    }]);
    rows.push([{
      text: editorLanguage(languageCode) === 'ar' ? '📂 إدارة البلوكات الداخلية' : '📂 Manage inner blocks',
  ...
  if (block.type === 'table') {
    rows.push([{
      text: editorLanguage(languageCode) === 'ar' ? '⚙️ إعدادات خلايا الجدول' : '⚙️ Table cell settings',
  ...
  if (block.type === 'list' && String(block?.data?.kind || '') === 'checklist') {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      let label = String(item.text || (editorLanguage(languageCode) === 'ar' ? 'مهمة' : 'Task'));
  ...
  if (MEDIA_CAPTION_TYPES.includes(String(block.type))) {
    rows.push([
      {
        text: editorLanguage(languageCode) === 'ar' ? 'تعديل الوصف' : 'Edit caption',
        callback_data: 'r:f:' + id + ':caption',
      },
      {
        text: editorLanguage(languageCode) === 'ar' ? 'تعديل المصدر' : 'Edit source',
        callback_data: 'r:f:' + id + ':credit',
      },
    ]);
  } else if (QUOTE_TYPES.includes(String(block.type))) {
    rows.push([{
      text: editorLanguage(languageCode) === 'ar' ? 'تعديل الكاتب' : 'Edit author',
      callback_data: 'r:f:' + id + ':credit',
    }]);
  }
  if (block.type !== 'anchor') {
...
export function buildTableOptionsKeyboard(blockId, languageCode) {
  const ar = editorLanguage(languageCode) === 'ar';
  const choices = ar
    ? [['تظليل خلية', 'sh'], ['إلغاء تظليل خلية', 'uh'], ['توسيط خلية', 'ce'], ['إلغاء توسيط خلية', 'ue'],
       ['تظليل الكل', 'sha'], ['إلغاء تظليل الكل', 'uha'], ['توسيط الكل', 'cea'], ['إلغاء توسيط الكل', 'uea']]
    : [['Shade cell', 'sh'], ['Unshade cell', 'uh'], ['Center cell', 'ce'], ['Uncenter cell', 'ue'],
       ['Shade all', 'sha'], ['Unshade all', 'uha'], ['Center all', 'cea'], ['Uncenter all', 'uea']];
  const rows = [];
  for (let index = 0; index < choices.length; index += 2) {
  ...
  rows.push([{
    text: ar ? '🧱 إعدادات مظهر الجدول' : '🧱 Table appearance',
    callback_data: 'r:tdisplay:' + blockId,
    style: 'primary',
  }]);
  rows.push([{ text: editorCopy(languageCode).back, callback_data: 'r:b:' + blockId }]);
  return { inline_keyboard: rows };
}
...
export function buildTableDisplayKeyboard(block, languageCode) {
  ...
  return {
    inline_keyboard: [
      [{ text: (tableFlag(block, 'is_bordered') ? '✅ ' : '❌ ') + (ar ? 'الحدود' : 'Borders'), callback_data: 'r:ttoggle:' + id + ':is_bordered' }],
      [{ text: (tableFlag(block, 'is_striped') ? '✅ ' : '❌ ') + (ar ? 'صفوف مخططة' : 'Striped rows'), callback_data: 'r:ttoggle:' + id + ':is_striped' }],
      [{ text: (tableFlag(block, 'is_compact') ? '✅ ' : '❌ ') + (ar ? 'وضع مضغوط' : 'Compact mode'), callback_data: 'r:ttoggle:' + id + ':is_compact' }],
      [{ text: hasCaption ? (ar ? 'تعديل عنوان الجدول' : 'Edit table caption') : (ar ? 'إضافة عنوان للجدول' : 'Add table caption'), callback_data: 'r:tcaption:' + id }],
      [{ text: editorCopy(languageCode).back, callback_data: 'r:tm:' + id }],
    ],
  };
}
...
export function buildDetailsContentKeyboard(childCount, languageCode) {
  const ar = editorLanguage(languageCode) === 'ar';
  const rows = [[{
    text: ar ? '➕ إضافة بلوك داخلي' : '➕ Add inner block',
    callback_data: 'r:details:add',
    style: 'primary',
  }]];
  if (Number(childCount) > 0) {
    rows.push([{
      text: (ar ? '✅ إنهاء' : '✅ Finish') + ' (' + Number(childCount) + ')',
      callback_data: 'r:details:finish',
      style: 'success',
    }]);
  }
  rows.push([{ text: editorCopy(languageCode).back, callback_data: 'r:details:cancel' }]);
  return { inline_keyboard: rows };
}
...
export function buildDetailsInnerBlockKeyboard(details, child, languageCode) {
  ...
  const ar = editorLanguage(languageCode) === 'ar';
  const rows = [[{ text: ar ? '👁 معاينة' : '👁 Preview', callback_data: 'r:dip:' + prefix, style: 'primary' }]];
  if (child?.type !== 'divider') rows.push([{ text: copyFor(languageCode, 'edit'), callback_data: 'r:die:' + prefix }]);
  if (MEDIA_CAPTION_TYPES.includes(String(child?.type || ''))) {
    rows.push([
      { text: ar ? 'الوصف' : 'Caption', callback_data: 'r:dif:' + prefix + ':caption' },
      { text: ar ? 'المصدر' : 'Credit', callback_data: 'r:dif:' + prefix + ':credit' },
    ]);
  } else if (QUOTE_TYPES.includes(String(child?.type || ''))) {
    rows.push([{ text: ar ? 'الكاتب' : 'Credit', callback_data: 'r:dif:' + prefix + ':credit' }]);
  } else if (!['footer', 'divider', 'anchor'].includes(String(child?.type || ''))) {
    rows.push([{ text: ar ? 'إضافة تذييل' : 'Add footer', callback_data: 'r:dif:' + prefix + ':add_footer' }]);
  }
  rows.push([{ text: editorCopy(languageCode).delete, callback_data: 'r:did:' + prefix, style: 'danger' }]);
  rows.push([
    position <= 0 ? { text: editorCopy(languageCode).moveUp, disabled: {} } : { text: editorCopy(languageCode).moveUp, callback_data: 'r:dimu:' + prefix },
    position >= children.length - 1 ? { text: editorCopy(languageCode).moveDown, disabled: {} } : { text: editorCopy(languageCode).moveDown, callback_data: 'r:dimd:' + prefix },
  ]);
  rows.push([{ text: editorCopy(languageCode).back, callback_data: 'r:dim:' + detailsId }]);
  return { inline_keyboard: rows };
}
...
export function anchorTargetPrompt(block, languageCode) {
  ...
  return editorLanguage(languageCode) === 'ar'
    ? 'اختر البلوك الذي يجب أن تؤدي إليه المرساة «' + name + '»:'
