...
async function askForButtonUser(userId, chatId, pending, code) {
  ...
  const userLabel = isArabic(code) ? '👤 رابط مستخدم · ' + title : '👤 User link · ' + title;
  const channelLabel = isArabic(code) ? '📢 اختر قناة · ' + title : '📢 Select channel · ' + title;
    ...
    if (!username) {
      try {
        ...
        await api.sendMessage({
          chat_id: message.chat.id,
          text: isArabic(code)
            ? 'تعذر الوصول إلى المستخدم. اختاره مرة ثانية.'
            : 'Could not access that user. Select again.',
        });
        await askForButtonUser(message.from.id, message.chat.id, pending, code);
        return true;
      }
...
async function previewBlock(query, session, block) {
  ...
  try {
    ...
    try {
      ...
      sent = await api.sendRichMessage({
        chat_id: query.from.id,
        rich_message: buildInputRichMessage([{ ...block, position: 0 }, context]),
      });
    }
...
async function previewResult(query, session) {
  ...
  if (latest && latest.state === 'managing' && latest.currentBlockId == null) {
