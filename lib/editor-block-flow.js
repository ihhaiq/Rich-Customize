...
async function showDashboard(query, session) {
  ...
  const text = updated.blocks?.length
...
export async function handleEditorBlockCallback(query) {
  ...
  if (data.startsWith('r:m:')) {
    ...
    await editMessage(
      chatId,
      messageId,
      isArabic(code) ? 'اختر الموقع الجديد:' : 'Choose the new position:',
  ...
  if (data.startsWith('r:mt:')) {
    ...
    try {
      ...
      if (!Number.isInteger(target)) {
        await api.answerCallbackQuery({
          callback_query_id: query.id,
          text: isArabic(code) ? 'الموقع الجديد غير صالح.' : 'Invalid new position.',
          show_alert: true,
        });
        return true;
      }
      ...
      if (!moveBlock(blocks, id, target)) {
        await api.answerCallbackQuery({
          callback_query_id: query.id,
          text: isArabic(code)
            ? 'تعذر نقل الجزء؛ ربما تغيرت الجلسة.'
            : 'Could not move the block; the session may have changed.',
          show_alert: true,
        });
        return true;
      }
