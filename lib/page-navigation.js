...
import { legacyStates, pageNavigationSessions, richPages } from 'schema';
...
import { getGuestMessage } from 'lib/guest-messages';
...
async function importLegacyNavigation(token) {
  const id = String(token || '');
  if (!id) return null;

  const legacy = await db.select({ payload: legacyStates.payload })
    .from(legacyStates)
    .where(eq(legacyStates.namespace, 'page_navigation')).get();
  ...
  const previousStack = value.previous_stack ?? value.previousStack ?? null;
  const updatedAt = Number(value.updated_at ?? value.updatedAt ?? 0);
  if (
    !Number.isSafeInteger(userId)
    || !Array.isArray(stack)
    || !stack.every((item) => typeof item === 'string')
    || (previousStack != null && (
      !Array.isArray(previousStack)
      || !previousStack.every((item) => typeof item === 'string')
    ))
    || !Number.isFinite(updatedAt)
    || updatedAt < nowSeconds() - NAVIGATION_TTL_SECONDS
  ) {
    return null;
  }
  ...
  const record = {
    ...
    userId,
    stack: [...stack],
    previousStack: previousStack == null ? null : [...previousStack],
    externalRoot: value.external_root ?? value.externalRoot ? 1 : 0,
    updatedAt,
  };
  await db.insert(pageNavigationSessions).values(record).onConflictDoNothing({
...
async function getNavigation(token) {
  ...
  let row = await db.select().from(pageNavigationSessions)
    .where(eq(pageNavigationSessions.token, String(token))).get();
  if (!row) row = await importLegacyNavigation(token);
...
async function back(token, userId) {
  ...
  let row = await db.select().from(pageNavigationSessions)
    .where(eq(pageNavigationSessions.token, String(token))).get();
  if (!row) {
    await getNavigation(token);
    row = await db.select().from(pageNavigationSessions)
      .where(eq(pageNavigationSessions.token, String(token))).get();
  }
...
function navigationButtons(nav, code) {
  ...
  if (canGoBack(nav)) {
    buttons.push({
      text: t(locale(code), 'ux.common.back'),
      callback_data: 'r:pback:' + nav.token,
    });
  }
  if (canGoHome(nav)) {
    buttons.push({
      text: t(locale(code), 'navigation.home'),
...
function errorText(_error, code) {
  return tr(locale(code), 'Could not open the page.');
}

async function callbackContext(query) {
  const message = query?.message;
  if (message?.chat?.id) {
    return {
      chatId: Number(message.chat.id),
      chatType: String(message.chat.type || ''),
      ephemeralMessageId: message.ephemeral_message_id || null,
      fromMessage: true,
    };
  }
  const guest = await getGuestMessage(query?.inline_message_id || '');
  if (!guest) return null;
  return {
    chatId: guest.chatId,
    chatType: guest.chatType,
    ephemeralMessageId: null,
    fromMessage: false,
  };
}

async function sendOrReplacePage(query, context, richMessage, replyMarkup) {
  if (!context) throw new Error('Guest message context is unavailable');
  const chatId = context.chatId;
  const chatType = context.chatType;

  if (context.ephemeralMessageId) {
    return api.editEphemeralMessageText({
      chat_id: chatId,
      receiver_user_id: query.from.id,
      ephemeral_message_id: context.ephemeralMessageId,
...
async function renderNavigationPage(query, pageId, nav) {
  ...
  if (!page) {
    await api.answerCallbackQuery({
      callback_query_id: query.id,
      text: tr(locale(code), 'This page no longer exists or has expired.'),
      show_alert: true,
    });
    return false;
  }
  if (!query?.message?.chat?.id) {
    await api.answerCallbackQuery({
      callback_query_id: query.id,
      text: t(locale(code), 'navigation.message_missing'),
      show_alert: true,
    });
    return false;
  }
  try {
    ...
    await logError('page_navigation.render', error, {
...
async function restoreNavigationRoot(query, nav) {
  ...
  await api.answerCallbackQuery({
    callback_query_id: query.id,
    text: t(locale(code), 'navigation.original_above'),
    show_alert: true,
  });
  return false;
}
...
async function openPage(query, requireSubscription) {
  const code = languageCode(query);
  const context = await callbackContext(query);
  if (!context) {
    await api.answerCallbackQuery({
      callback_query_id: query.id,
      text: tr(locale(code), 'Could not determine the Guest message chat.'),
      show_alert: true,
    });
    return true;
  }
  ...
  if (
    requireSubscription
    && !await isChatSubscriber(context.chatId, query.from.id)
  ) {
    await api.answerCallbackQuery({
      callback_query_id: query.id,
      text: tr(locale(code), 'You are not subscribed.'),
      show_alert: true,
    });
    return true;
  }
  ...
  try {
    const payload = await pagePayload(page, nav, code);
    await sendOrReplacePage(query, context, payload.richMessage, payload.replyMarkup);
  } catch (error) {
    await logError('page_navigation.open', error, {
      userId: query?.from?.id,
      chatId: query?.message?.chat?.id,
      callbackData: query?.data,
    });
...
export async function handlePageNavigationCallback(query) {
  ...
  if (data.startsWith('r:popup:')) {
    ...
    await api.answerCallbackQuery({
      callback_query_id: query.id,
      text: value == null
        ? tr(locale(languageCode(query)), 'This alert is no longer available.')
        : value.slice(0, 200),
      show_alert: true,
    });
    return true;
  }
  if (data.startsWith('r:cbd:')) {
    return openPage({
      ...query,
      data: 'r:page:' + data.slice('r:cbd:'.length),
    }, false);
  }
  if (data.startsWith('r:cbds:')) {
    return openPage({
      ...query,
      data: 'r:spage:' + data.slice('r:cbds:'.length),
    }, true);
  }
  ...
  if (data.startsWith('r:pback:')) {
    ...
    if (!nav) {
      await api.answerCallbackQuery({
        callback_query_id: query.id,
        text: t(locale(languageCode(query)), 'navigation.expired'),
        show_alert: true,
      });
      return true;
    }
    ...
    try {
      ...
      await logError('page_navigation.back', error, {
        userId: query?.from?.id,
        chatId: query?.message?.chat?.id,
        callbackData: query?.data,
      });
  ...
  if (data.startsWith('r:phome:')) {
    ...
    try {
      ...
      await logError('page_navigation.home', error, {
        userId: query?.from?.id,
        chatId: query?.message?.chat?.id,
        callbackData: query?.data,
      });
  ...
  if (data === 'r:ephemeral:restore') {
    if (!query.message?.chat?.id || !query.message?.ephemeral_message_id) {
      await api.answerCallbackQuery({
        callback_query_id: query.id,
        text: tr(locale(languageCode(query)), 'The original message is unavailable.'),
        show_alert: true,
      });
      return true;
    }
    try {
      ...
      await logError('page_navigation.restore', error, {
        userId: query?.from?.id,
        chatId: query?.message?.chat?.id,
        callbackData: query?.data,
      });
