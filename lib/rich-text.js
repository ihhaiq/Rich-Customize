...
function normalizeButtonUrl(value) {
  ...
  try {
    ...
    if (url.protocol === 'tg:' && (url.host || url.pathname)) return raw;
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return null;
    const hostname = url.hostname || '';
    if (!hostname.includes('.')) {
      if (!url.pathname.replaceAll('/', '') && /^[A-Za-z0-9_]{5,32}$/.test(hostname)) {
...
function buttonPayload(title, buttonType, value, color, audience = 'all') {
  ...
  if (buttonType === 'url') {
    ...
    if (!value || new TextEncoder().encode(callback).length > 64) return null;
