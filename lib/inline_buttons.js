...
export function findUserButtonMarkers(text){
  const out=[];const source=String(text??'');MARKER_RE.lastIndex=0;
  for(const match of source.matchAll(MARKER_RE)){const parts=markerParts(match[0]);if(parts?.buttonType==='user'&&!parts.value)out.push({marker:match[0],title:parts.title,color:parts.color});}
  return out;
}
export function resolveUserButtonMarker(blocks,marker,userId,username=null){
  const parts=markerParts(marker);if(!parts)return blocks;
  const target=username?'https://t.me/'+String(username).replace(/^@/,''):'tg://user?id='+Number(userId);
  const replacement='{'+parts.title+':url '+target+(parts.color?'#'+parts.color:'')+'}';
  const walk=value=>{
    if(typeof value==='string')return value.replace(marker,replacement);
    if(Array.isArray(value))return value.map(walk);
    if(value&&typeof value==='object')return Object.fromEntries(Object.entries(value).map(([k,v])=>[k,walk(v)]));
    return value;
  };
  for(let i=0;i<blocks.length;i++)blocks[i]=walk(blocks[i]);
  return blocks;
}
