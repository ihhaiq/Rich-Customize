...
function buildEmptyErrorTestRichMessage(isoTime) {
  const cells = [
    [tableCell('التفصيل', true), tableCell('القيمة', true)],
    [tableCell('المصدر'), tableCell('—')],
    [tableCell('الخطأ'), tableCell('—')],
    [tableCell('Update ID'), tableCell('—')],
    [tableCell('Chat ID'), tableCell('—')],
    [tableCell('Callback'), tableCell('—')],
    [tableCell('السياق'), tableCell('—')],
  ];
  return {
    blocks: [
      { type: 'heading', text: '🚨 اختبار مراقب الأخطاء', size: 1 },
      { type: 'table', cells, is_bordered: true, is_compact: false },
      { type: 'footer', text: 'اختبار فقط · ' + isoTime },
    ],
    is_rtl: true,
  };
}

export async function testErrorLogTemplate() {
  const config = await readConfig();
  if (!config.enabled || config.chatId == null) {
    throw new Error('قناة سجل الأخطاء غير مفعلة.');
  }
  await api.sendRichMessage({
    chat_id: config.chatId,
    rich_message: buildEmptyErrorTestRichMessage(new Date().toISOString()),
    disable_notification: true,
  });
  return true;
}
