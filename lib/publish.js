...
function ar(c){return String(c||'').toLowerCase().startsWith('ar');}
function t(c){
  if(ar(c))return {
    create:'إنشاء منشور',none:'لا توجد قناة أو مجموعة مشتركة يكون فيها المستخدم والبوت مشرفين.',
    addHint:'أضف البوت من أحد الزرين، وبعد نجاح الإضافة سيصلك إشعار هنا.',
    choose:'اضغط على كل قناة أو مجموعة لتحديدها للإرسال المتعدد.',
    selected:'المحدد حالياً: ',select:'تحديد',settings:'الإعدادات والإرسال',
    addChannel:'إضافة إلى قناة',addGroup:'إضافة إلى مجموعة',back:'🔙 رجوع',
    silentOn:'✅ إرسال صامت',silentOff:'❌ إرسال صامت',protectedOn:'✅ حماية المحتوى',protectedOff:'❌ حماية المحتوى',
    send:(n)=>'إرسال ('+n+')',settingsTitle:'إعدادات المنشور',selectedChats:'المحادثات المحددة: ',
    settingsHint:'اختر الإعدادات ثم اضغط إرسال:',confirm:(n)=>'تأكيد إرسال المنشور إلى '+n+' محادثة؟',
    yes:'✅ نعم، إرسال',cancel:'إلغاء',needChat:'حدد محادثة واحدة على الأقل.',
    invalidChat:'اختيار محادثة غير صالح.',unavailable:'المحادثة لم تعد متاحة، أو أن صلاحيات أحد المشرفين تغيرت.',
    selectedOn:'تم تحديد المحادثة للإرسال',selectedOff:'تم إلغاء تحديد المحادثة',sending:'جاري إرسال المنشور…',
    result:'نتيجة الإرسال:',ok:'نجح: ',fail:'فشل: ',again:'يمكنك تغيير الإعدادات وإرسال المنشور مرة أخرى.',
    shortened:'… تم اختصار قائمة النتائج',reason:'سبب الفشل: ',reached:(title)=>'✅ تم الوصول إلى «'+title+'».',
    noPostRight:'تمت إضافة البوت، لكن بدون صلاحية نشر الرسائل في القناة.',publish:'📤 نشر',
  };
  return {
    create:'Create post',none:'There is no group or channel where both you and the bot are administrators.',
    addHint:'Add the bot using one of the buttons. This panel updates after access is granted.',
    choose:'Select every group or channel you want to publish to.',selected:'Selected: ',select:'Select',
    settings:'Settings & send',addChannel:'Add to channel',addGroup:'Add to group',back:'🔙 Back',
    silentOn:'✅ Silent',silentOff:'❌ Silent',protectedOn:'✅ Protect content',protectedOff:'❌ Protect content',
    send:(n)=>'Send ('+n+')',settingsTitle:'Post settings',selectedChats:'Selected chats: ',
    settingsHint:'Choose settings, then send:',confirm:(n)=>'Confirm publishing to '+n+' chat(s)?',
    yes:'✅ Yes, send',cancel:'Cancel',needChat:'Select at least one chat.',invalidChat:'Invalid chat selection.',
    unavailable:'The chat is no longer available, or administrator permissions changed.',
    selectedOn:'Chat selected',selectedOff:'Chat unselected',sending:'Publishing…',result:'Send result:',
    ok:'Succeeded: ',fail:'Failed: ',again:'You can change settings and send the post again.',
    shortened:'… result list shortened',reason:'Failure reason: ',reached:(title)=>'✅ Reached “'+title+'”.',
    noPostRight:'The bot was added, but it cannot post messages in this channel.',publish:'📤 Publish',
  };
}
...
async function clearPanel(userId){await db.delete(managedPublishPanels).where(eq(managedPublishPanels.userId,Number(userId))).run();}
...
function pickerRich(chats,selected,channelUrl,groupUrl,c){
  const cc=t(c),set=new Set(selected||[]),rows=[];
...
function settingsText(n,c){const cc=t(c);return cc.settingsTitle+'\n\n'+cc.selectedChats+n+'\n'+cc.settingsHint;}
function settingsRich(n,silent,protectedValue,c,body=null){
  const cc=t(c),text=body||settingsText(n,c),rows=[
...
function confirmRich(n,c){const cc=t(c);return {blocks:[{type:'paragraph',text:cc.confirm(n)},{type:'table',cells:[[cell(richButton(cc.yes,{callback_data:'r:postsend',style:'success'}),2)]],is_bordered:true,is_compact:true}]};}
function backKeyboard(data='r:back',c='en'){return {inline_keyboard:[[{text:t(c).back,callback_data:data}]]};}
...
async function refreshPanel(userId){
  ...
  const [channelUrl,groupUrl]=await addLinks();
  try{await api.editMessageText({chat_id:p.chatId,message_id:p.messageId,rich_message:pickerRich(chats,safe,channelUrl,groupUrl,'en'),reply_markup:backKeyboard('r:back','en')});}
  catch{}
}
...
export async function handlePublishCallback(query){
  ...
  const userId=query?.from?.id,c=code(query),cc=t(c);
  ...
  if(data==='r:postsettings'){
    ...
    await updateEditorSession(userId,{postSelectedChatIds:selected});await clearPanel(userId);
...
export async function handleMyChatMember(update){
  ...
  if(chatType==='channel'&&!Boolean(newMember?.can_post_messages)){
    if(actor?.id){try{await api.sendMessage({chat_id:actor.id,text:t(actor.language_code).noPostRight});}catch{}}
    return;
  }
  if(!actor?.id||actor?.is_bot)return;
  await rememberChat(actor.id,chat);
  await refreshPanel(actor.id);
  try{await api.sendMessage({chat_id:actor.id,text:t(actor.language_code).reached(chat.title||String(chat.id)),reply_markup:{inline_keyboard:[[{text:t(actor.language_code).publish,callback_data:'r:postchat:'+chat.id,style:'success'}]]}});}catch{}
}
