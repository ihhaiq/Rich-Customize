...
function copy(languageCode) {
  ...
  return {
    locale,
    title: t(locale, 'ux.buttons.title'),
    count: (n) => t(locale, 'ux.buttons.count', { count: n }),
    empty: t(locale, 'ux.buttons.empty'),
    choose: t(locale, 'common.choose_action'),
    add: t(locale, 'ux.buttons.add'),
    layout: (n) => t(locale, 'ux.buttons.layout', { count: n }),
    layoutCustom: t(locale, 'ux.buttons.layout_custom'),
    preview: t(locale, 'button_preview'),
    undo: t(locale, 'ux.buttons.undo'),
    back: t(locale, 'ux.common.back'),
    editTitle: t(locale, 'ux.buttons.edit_title'),
    editValue: t(locale, 'ux.buttons.edit_value'),
    editType: t(locale, 'ux.buttons.edit_type'),
    color: t(locale, 'color'),
    reorder: t(locale, 'reorder'),
    delete: t(locale, 'ux.buttons.delete'),
    yesDelete: t(locale, 'ux.common.yes_delete'),
    cancel: t(locale, 'ux.common.cancel'),
    added: t(locale, 'ux.buttons.added'),
    deleted: t(locale, 'ux.buttons.deleted'),
    missing: t(locale, 'ux.buttons.missing'),
    invalid: t(locale, 'invalid'),
    sendFormat: t(locale, 'buttons.send_format', { label: '{ ' + t(locale, 'buttons.format_parts') + ' }' }),
    invalidFormat: t(locale, 'buttons.invalid_format'),
    sendTitle: t(locale, 'ux.buttons.send_new_title'),
    sendValue: t(locale, 'ux.buttons.send_new_value'),
    invalidText: tr(locale, 'Send a valid text value.'),
    longTitle: tr(locale, 'The button title is too long; the maximum is 64 characters.'),
    max: tr(locale, 'You reached the maximum number of buttons.'),
    noPreview: tr(locale, 'There are no buttons to preview.'),
    previewText: t(locale, 'button_preview'),
    closedPreview: tr(locale, 'Preview closed'),
    typeChanged: tr(locale, '✅ Button type changed.'),
    titleChanged: tr(locale, '✅ Button title changed.'),
    valueChanged: tr(locale, '✅ Button content changed.'),
    valueTypeChanged: tr(locale, '✅ Button content and type changed automatically.'),
    styleChanged: tr(locale, 'Color changed'),
    positionChanged: tr(locale, 'Order changed'),
    pageLinked: (title) => tr(locale, '✅ Button linked to page “{title}”.', { title }),
    pageMissing: tr(locale, 'The page code does not exist or does not belong to you.'),
    savePageFirst: tr(locale, 'Save a page first so you can link the button to it.'),
    choosePage: tr(locale, 'Choose the page opened by this button:'),
    editing: (title) => t(locale, 'ux.buttons.editing', { title }),
    currentType: (type) => t(locale, 'ux.buttons.current_type', { type }),
    types: {
      url: t(locale, 'ux.buttons.type.url'),
      callback_data: t(locale, 'ux.buttons.type.callback'),
      copy: t(locale, 'ux.buttons.type.copy'),
      popup: t(locale, 'ux.buttons.type.popup'),
      web_app: t(locale, 'ux.buttons.type.web_app'),
      login_url: t(locale, 'ux.buttons.type.login_url'),
      switch_inline: t(locale, 'ux.buttons.type.inline'),
      switch_inline_current: t(locale, 'ux.buttons.type.inline_here'),
      disabled: t(locale, 'ux.buttons.type.disabled'),
      page: t(locale, 'ux.buttons.type.page'),
    },
    styles: {
      default: t(locale, 'ux.buttons.style.default'),
      primary: t(locale, 'ux.buttons.style.primary'),
      success: t(locale, 'ux.buttons.style.success'),
      danger: t(locale, 'ux.buttons.style.danger'),
    },
    layoutHint: t(locale, 'ux.buttons.layout_hint'),
    customHint: t(locale, 'ux.buttons.custom_hint'),
    custom: t(locale, 'ux.buttons.custom'),
    row: (n) => t(locale, 'ux.buttons.row_number', { number: n }),
  };
}
...
function validateValue(type,value,languageCode) {
  const c=copy(languageCode), locale=c.locale;
  if (type==='disabled') return ['',null];
  if (type==='url') {
    const v=normalizeUrl(value);
    return v && v.length<=256 ? [v,null] : [null, t(locale, 'ux.buttons.invalid_url')];
  }
  if (type==='web_app'||type==='login_url') {
    const v=normalizeUrl(value);
    return v?.startsWith('https://') && v.length<=256 ? [v,null] : [null, tr(locale, 'This type requires a valid HTTPS URL.')];
  }
  if (type==='page') return /^[A-Za-z0-9_-]{1,64}$/.test(String(value||'').trim())
    ? [String(value).trim(),null] : [null,c.pageMissing];
  if (type==='copy' && String(value).length>256) return [null,tr(locale, 'The copy text is too long; the maximum is 256 characters.')];
  if (type==='callback_data' && !(utf8Len(value)>=1 && utf8Len(value)<=64)) return [null,tr(locale, 'The callback_data value must be between 1 and 64 bytes.')];
  if (type==='popup' && String(value).length>200) return [null,tr(locale, 'The alert text is too long; the maximum is 200 characters.')];
  if (type==='switch_inline'||type==='switch_inline_current') {
    const v=String(value).trim().toLowerCase()==='/empty'?'':String(value);
    return v.length<=256?[v,null]:[null,tr(locale, 'The inline query is too long; maximum 256 characters.')];
  }
...
function managerKeyboard(buttons,width,languageCode) {
  ...
  const out=ordered.map((b,i)=>[{text:(i+1)+'. '+String(b.text||'—').slice(0,24)+' · '+typeLabel(b,c)+' · '+styleLabel(b,c),callback_data:'r:bed:'+b.id}]);
...
function editorText(button,languageCode){const c=copy(languageCode);return [c.editing(button.text||'—'),c.currentType(typeLabel(button,c)),'',c.choose].join('\n');}
function pickerKeyboard(buttons,action,languageCode) {
  const c=copy(languageCode), ordered=normalize(buttons);
  const out=ordered.map((button,index)=>[{
    text:String(index+1)+'. '+String(button.text||'—'),
    callback_data:'r:bt:'+action+':'+button.id,
  }]);
  out.push([{text:c.back,callback_data:'r:buttons'}]);
  return {inline_keyboard:out};
}
...
function positionKeyboard(buttons,id,languageCode) {
  ...
  const result=ordered.map((b,i)=>[{text:(b===current?'✅ ':'')+(i+1)+' — '+String(b.text||'—'),...(b===current?{disabled:{}}:{callback_data:'r:bmv:'+id+':'+i})}]);
...
function promptForType(type,languageCode) {
  const locale = resolveLanguage(languageCode);
  return {
    url: tr(locale, 'Send the new URL; @username is accepted.'),
    callback_data: tr(locale, 'Send callback_data; maximum 64 bytes.'),
    copy: tr(locale, 'Send the text to copy; maximum 256 characters.'),
    popup: tr(locale, 'Send popup text; maximum 200 characters.'),
    web_app: tr(locale, 'Send an HTTPS Web App URL.'),
    login_url: tr(locale, 'Send an HTTPS URL from the domain connected in BotFather.'),
    switch_inline: tr(locale, 'Send an Inline query, or /empty.'),
    switch_inline_current: tr(locale, 'Send an Inline query for this chat, or /empty.'),
  }[type];
}
...
export async function handleEditorButtonCallback(query) {
  ...
  const relevant=data==='r:buttons'||data==='r:ba'||data==='r:brow'||data==='r:bpreview'||data==='r:bpback'
    ...
    ||data.startsWith('r:bmv:')||data.startsWith('r:bpg:')||data.startsWith('r:bs:')||data.startsWith('r:bt:')||data.startsWith('r:browset:')
  ...
  if(data.startsWith('r:bdel:')){
    ...
    if(!b){await api.answerCallbackQuery({callback_query_id:query.id,text:c.missing,show_alert:true});return true;}
    await edit(query,t(c.locale, 'ux.buttons.delete_confirm', { title: String(b.text||'—') }),deleteKeyboard(id,languageCode));
    await api.answerCallbackQuery({callback_query_id:query.id});return true;
  }
  if(data.startsWith('r:bs:')){
    const action=data.slice('r:bs:'.length);
    if(!['delete','style','move','value','url','title','type'].includes(action)){
      await api.answerCallbackQuery({callback_query_id:query.id,text:c.invalid,show_alert:true});return true;
    }
    if(!(session.messageButtons||[]).length){
      await api.answerCallbackQuery({callback_query_id:query.id,text:c.empty,show_alert:true});return true;
    }
    const labels={
      delete:tr(c.locale,'Choose the button to remove:'),
      style:tr(c.locale,'Choose the button whose color you want to change:'),
      move:tr(c.locale,'Choose the button to reorder:'),
      value:tr(c.locale,'Choose the button whose value you want to change:'),
      url:tr(c.locale,'Choose the button whose value you want to change:'),
      title:tr(c.locale,'Choose the button whose title you want to change:'),
      type:tr(c.locale,'Choose the button whose type you want to change:'),
    };
    await edit(query,labels[action],pickerKeyboard(clone(session.messageButtons||[]),action,languageCode));
    await api.answerCallbackQuery({callback_query_id:query.id});return true;
  }
  if(data.startsWith('r:bt:')){
    const parts=data.split(':'),action=parts[2],id=parts[3],button=getButton(session.messageButtons||[],id);
    if(!button){await api.answerCallbackQuery({callback_query_id:query.id,text:c.missing,show_alert:true});return true;}
    if(action==='delete'){
      await edit(query,t(c.locale, 'ux.buttons.delete_confirm', { title: String(button.text||'—') }),deleteKeyboard(id,languageCode));
    }else if(action==='style'){
      await edit(query,c.editing(button.text||'—'),styleKeyboard(id,String(button.style||'default'),languageCode));
    }else if(action==='move'){
      await edit(query,c.editing(button.text||'—'),positionKeyboard(clone(session.messageButtons||[]),id,languageCode));
    }else if(action==='type'){
      await updateEditorSession(userId,{currentButtonId:id});
      await edit(query,c.editing(button.text||'—'),typeKeyboard('r:bct:'+id,languageCode));
    }else if(['value','url','title'].includes(action)){
      const pending=action==='title'?'title':'value';
      await prompt(query,userId,pending==='title'?c.sendTitle:c.sendValue,{pendingButtonAction:pending,currentButtonId:id});
    }else{
      await api.answerCallbackQuery({callback_query_id:query.id,text:c.invalid,show_alert:true});return true;
    }
    await api.answerCallbackQuery({callback_query_id:query.id});return true;
  ...
  if(data.startsWith('r:bedit:')){
    ...
    if(!b){await api.answerCallbackQuery({callback_query_id:query.id,text:c.missing,show_alert:true});return true;}
    if(action==='style')await edit(query,c.editing(b.text||'—'),styleKeyboard(id,String(b.style||'default'),languageCode));
    else if(action==='move')await edit(query,c.editing(b.text||'—'),positionKeyboard(clone(session.messageButtons||[]),id,languageCode));
    else if(action==='type'){await updateEditorSession(userId,{currentButtonId:id});await edit(query,c.editing(b.text||'—'),typeKeyboard('r:bct:'+id,languageCode));}
...
export async function handleEditorButtonMessage(message) {
  ...
  if(action==='add_spec'){
    ...
    }else{await api.sendMessage({chat_id:message.chat.id,text:tr(c.locale,'Button editing ended. Return to the manager and try again.')});await updateEditorSession(userId,{state:'managing'});return true;}
  }
