...
function pickerKeyboard(buttons,action,languageCode) {
  ...
  const out=ordered.map((button,index)=>[{
    text:String(index+1)+'. '+String(button.text||'Button'),
...
export async function handleEditorButtonCallback(query) {
  ...
  if(data.startsWith('r:bdel:')){
    ...
    if(!b){await api.answerCallbackQuery({callback_query_id:query.id,text:c.missing,show_alert:true});return true;}
    await edit(query,t(c.locale, 'ux.buttons.delete_confirm', { title: String(b.text||'Button') }),deleteKeyboard(id,languageCode));
    await api.answerCallbackQuery({callback_query_id:query.id});return true;
  }
  ...
  if(data.startsWith('r:bt:')){
    ...
    if(action==='delete'){
      await edit(query,t(c.locale, 'ux.buttons.delete_confirm', { title: String(button.text||'Button') }),deleteKeyboard(id,languageCode));
    }else if(action==='style'){
      await edit(query,c.editing(button.text||'Button'),styleKeyboard(id,String(button.style||'default'),languageCode));
    }else if(action==='move'){
      await edit(query,c.editing(button.text||'Button'),positionKeyboard(clone(session.messageButtons||[]),id,languageCode));
    }else if(action==='type'){
      await updateEditorSession(userId,{currentButtonId:id});
      await edit(query,c.editing(button.text||'Button'),typeKeyboard('r:bct:'+id,languageCode));
