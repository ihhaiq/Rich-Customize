...
async function ownerPageCount(ownerId) {
  const rows = await db.select({ pageId: richPages.pageId })
    .from(richPages)
...
async function persistPage(userId, title, session, existingId = null) {
  ...
  if (existingId) {
    ...
    await db.update(richPages).set({
      ...
      buttons: JSON.parse(JSON.stringify(session.messageButtons || [])),
...
export async function handleEditorPageCallback(query) {
  ...
  if (data.startsWith('r:pageopen:')) {
    ...
    const updated = await updateEditorSession(query.from.id, {
      ...
      messageButtons: JSON.parse(JSON.stringify(page.buttons || [])),
...
export async function handleEditorPageMessage(message) {
  ...
  if (!session || session.state !== 'saving_page_name') return false;
  ...
  try {
    ...
    let id;
    try {
      id = await persistPage(
        userId,
        title,
        latest,
        latest.currentPageId || null,
      );
      if (!id) {
        id = await persistPage(userId, title, {
          ...latest,
          currentPageId: null,
        }, null);
      }
