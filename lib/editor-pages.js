...
function copy(code) {
  if (isArabic(code)) {
    return {
      noBlocks: 'لا توجد أجزاء لحفظها.',
      sendName: 'أرسل اسم الصفحة لحفظها؛ الحد الأقصى 64 حرفًا.',
      originalMissing: 'الصفحة الأصلية لم تعد موجودة.',
      originalMissingPrompt: 'الصفحة الأصلية لم تعد موجودة. أرسل اسمًا لحفظها كصفحة جديدة.',
      badName: 'اسم الصفحة يجب أن يكون نصًا.',
      longName: 'اسم الصفحة طويل جدًا؛ الحد الأقصى 64 حرفًا.',
      limit: 'وصلت للحد الأقصى للصفحات المحفوظة: ' + MAX_SAVED_PAGES + '.',
      updatedNotice: (title) => '✅ تم تحديث الصفحة المحفوظة «' + title + '».',
      updatedToast: '✅ تم حفظ التعديلات بنفس الكود',
      savedNotice: (title) => '✅ تم حفظ الصفحة «' + title + '».',
      savedMessage: (id, updated) => (updated ? '✅ تم تحديث الصفحة المحفوظة.\n\nالكود: ' : '✅ تم حفظ الصفحة.\n\nالكود: ')
        + id + '\n\nتقدر تستعمله داخل النص هكذا:\n{التالي:cbd ' + id + '#b}'
        + '\n\nأو اختَر «CBD — فتح صفحة» من قائمة الأزرار.',
      opened: 'تم فتح الصفحة',
      missing: 'الصفحة محذوفة أو لا تخصك.',
      expired: 'انتهت الجلسة. أرسل /editor للبدء من جديد.',
      renamePrompt: (title) => 'أرسل الاسم الجديد للصفحة «' + title + '».',
      deleteConfirm: (title) => 'هل تريد حذف الصفحة «' + title + '»؟',
      deletedRecoverable: 'تم حذف الصفحة. تكدر تستعيدها قبل ما تكمل.',
      deleted: 'تم حذف الصفحة',
      restored: 'تمت استعادة الصفحة',
      restoreUnavailable: 'ما تكدر تستعيد هذه الصفحة.',
      invalid: 'اختيار غير صالح.',
    };
  }
  return {
    noBlocks: 'There are no blocks to save.',
    sendName: 'Send a page name to save it; maximum 64 characters.',
    originalMissing: 'The original page no longer exists.',
    originalMissingPrompt: 'The original page no longer exists. Send a name to save it as a new page.',
    badName: 'The page name must be text.',
    longName: 'The page name is too long; maximum 64 characters.',
    limit: 'You reached the saved-page limit: ' + MAX_SAVED_PAGES + '.',
    updatedNotice: (title) => '✅ Updated saved page “' + title + '”.',
    updatedToast: '✅ Saved changes with the same code',
    savedNotice: (title) => '✅ Saved page “' + title + '”.',
    savedMessage: (id, updated) => (updated ? '✅ Saved page changes.\n\nCode: ' : '✅ Page saved.\n\nCode: ')
      + id + '\n\nYou can use it inside text like this:\n{Next:cbd ' + id + '#b}'
      + '\n\nOr choose “CBD — Open page” from the buttons list.',
    opened: 'Page opened',
    missing: 'The page was deleted or does not belong to you.',
    expired: 'The session has expired. Send /editor to start again.',
    renamePrompt: (title) => 'Send the new name for page “' + title + '”.',
    deleteConfirm: (title) => 'Delete page “' + title + '”?',
    deletedRecoverable: 'Page deleted. You can restore it before continuing.',
    deleted: 'Page deleted',
    restored: 'Page restored',
    restoreUnavailable: 'This page can no longer be restored.',
...
function limitText(result, code) {
  ...
  return isArabic(code) ? 'تجاوز المحتوى أحد حدود المحرر.' : 'The content exceeds an editor limit.';
}
...
async function persistPage(userId, title, session, existingId = null) {
  ...
  const cleanedTitle = String(title || '').trim().slice(0, 64) || (isArabic(session?.languageCode) ? 'صفحة بلا اسم' : 'Untitled page');
  const stamp = nowSeconds();
  if (existingId) {
...
async function restorePage(userId, pageId, snapshot) {
  ...
  try {
    ...
    await db.insert(richPages).values({
      ...
      ownerId: Number(userId),
      title: String(snapshot.title || 'Untitled page').trim().slice(0, 64),
      blocks,
