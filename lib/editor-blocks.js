...
export function listData(plain, kind = 'bullet') {
  const safeKind = ['bullet', 'numbered', 'checklist'].includes(String(kind))
    ...
    : 'bullet';
  const items = [];
  for (const rawLine of lineItems(plain)) {
    ...
    if (safeKind === 'checklist') {
      ...
      line = line.replace(/^\d+\s*[.)-]\s*/u, '').trim();
    } else {
  ...
  const html = '<' + tag + '>' + items.map((item) => {
    const prefix = safeKind === 'checklist'
      ...
      : '';
    return '<li>' + prefix + escapeHtml(item.text) + '</li>';
  }).join('') + '</' + tag + '>';
  return { items, kind: safeKind, text: String(plain || ''), html };
}

export function tableData(plain) {
  const rows = lineItems(plain)
    .filter((line) => line.trim())
    .map((line) => line.split('|').map((cell) => cell.trim()));
  const widest = Math.max(1, ...rows.map((row) => row.length));
  const normalized = rows.map((row) => {
    if (row.length === 1 && widest > 1) return [{ text: row[0], colspan: widest }];
    return row;
  });
  ...
  return { rows: normalized, text: String(plain || ''), html };
}
