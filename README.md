...
### لوحة المطوّر والنسخ الاحتياطي

ضع رقم حساب المطوّر في متغير البيئة `DEVELOPER_ID`، أو عدة أرقام مفصولة بفواصل
في `DEVELOPER_IDS`. بعدها يعرض الأمر `/dev` زرًا لإنشاء ملف ZIP يحتوي ملفات
بيانات JSON الحالية فقط، بدون التوكن أو متغيرات البيئة:
