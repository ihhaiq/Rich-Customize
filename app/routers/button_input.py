...
from app.keyboards import build_buttons_manager_keyboard
...
async def receive_button_value(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    if not value:
        ...
        return
    if action == "title" and len(value) > 64:
    ...
    if action == "add_spec":
        specification = parse_message_button_spec(value)
        if specification is None:
            await message.answer(t("buttons.invalid_format"))
            return
        title, button_type, content = specification
        if len(title) > 64:
            await message.answer(tr("عنوان الزر طويل جدًا؛ الحد الأقصى 64 حرفًا."))
            return
        normalized_value, error = normalize_button_value(button_type, content)
        ...
        if button_type == "page":
            ...
            if page is None or int(page.get("owner_id", 0)) != user_id:
                await message.answer(tr("كود الصفحة غير موجود أو لا يخصك."))
                return
        button = add_message_button(
            buttons, title, normalized_value, button_type,
        )
        if button is None:
