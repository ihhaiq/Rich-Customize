// Beta 0.3.32 — one long-press model for every editor Block.
(() => {
  ...
  const PERFORMANCE_MODE = document.documentElement.classList.contains("mobile-performance") || COARSE_POINTER || (NO_HOVER && window.innerWidth <= 820);
  ...
  const PRESS_SELECTOR = ["button",".menu-item",".sheet-item",".soft-btn",".primary-soft",".fab",".icon-btn",".starter-card",".block"].join(",");
  const BLOCK_EXCLUSIVE_SELECTOR = ["button","a","[data-inline-rich-button]","[data-no-long-press]"].join(",");
  let active=null,longTimer=null,suppressClickUntil=0,suppressContextUntil=0,suppressTarget=null;

  function interactionModeBlocksLongPress(){const root=document.getElementById("editorView");return Boolean(root?.classList.contains("multi-select-mode")||root?.classList.contains("preview-mode"));}
  ...
  function clearNativeSelectionInside(block){try{const focused=document.activeElement;if(focused&&block.contains(focused)&&(focused.matches?.("input,textarea,select")||focused.isContentEditable))focused.blur?.();const selection=window.getSelection?.();if(selection&&!selection.isCollapsed)selection.removeAllRanges();}catch(_){}}
  function finishLongPress(el){el.classList.remove("is-pressed","long-pressing","long-pressed");if(PERFORMANCE_MODE){requestAnimationFrame(()=>el.classList.add("long-pressed"));setTimeout(()=>el.classList.remove("long-pressed"),300);}else{void el.offsetWidth;el.classList.add("long-pressed");setTimeout(()=>el.classList.remove("long-pressed"),520);}haptic("medium");if(el.classList.contains("block")&&el.dataset.id){try{clearNativeSelectionInside(el);selectBlock?.(el.dataset.id);const block=typeof current!=="undefined"?current?.blocks?.find?.(item=>String(item.id)===String(el.dataset.id)):null;if(block)openBlockMenu?.(block);}catch(_){}}suppressClickUntil=Date.now()+650;suppressContextUntil=Date.now()+900;suppressTarget=el;}

  document.addEventListener("pointerdown",event=>{
    if(interactionModeBlocksLongPress())return;if(event.button!==undefined&&event.button!==0)return;const el=pressTargetFrom(event.target);if(!el)return;clearTimer();active={el,pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,moved:false,longPressed:false,nativeControl:nativeControlTarget(event.target,el)};el.classList.add("press-surface","is-pressed");el.classList.remove("press-release","long-pressed");addRipple(el,event.clientX,event.clientY);if(!active.nativeControl){requestAnimationFrame(()=>{if(active?.el===el)el.classList.add("long-pressing");});longTimer=setTimeout(()=>{if(!active||active.el!==el||active.moved)return;active.longPressed=true;finishLongPress(el);},LONG_PRESS_MS);}
  },{passive:true});
  document.addEventListener("pointermove",event=>{if(!active||event.pointerId!==active.pointerId)return;const dx=event.clientX-active.startX,dy=event.clientY-active.startY;if(Math.hypot(dx,dy)>MOVE_CANCEL_PX){active.moved=true;clearTimer();active.el.classList.remove("long-pressing");if(Math.abs(dy)>6||Math.abs(dx)>8)active.el.classList.remove("is-pressed");}},{passive:true});
...
(() => {
  ...
  if(!document.querySelector('script[data-table-cell-tools]')){const script=document.createElement("script");script.src="/miniapp/static/table_cell_tools.js?v=0.3.32";script.dataset.tableCellTools="1";script.onload=()=>{try{if(typeof current!=="undefined"&&current?.blocks?.some?.(block=>block.type==="table"))renderBlocks?.();}catch(_){}};document.body.appendChild(script);}
})();
